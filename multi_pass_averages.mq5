...
input enumAveragesType AverageType   = avgSma;  // Calculation type
input int              AverageDepth  = 2;       // Depth
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,getAverageName(AverageType)+" ("+(string)AveragePeriod+")");
   return(0);
  }
...
//
   ...
   int depth=(int)MathMax(MathMin(AverageDepth,25),1);
   for(int i=(int)MathMax(prev_calculated-1,1); i<rates_total; i++)
     {
      double tprice=price[i]; if(tprice==EMPTY_VALUE) tprice=0;
      for(int k=0; k<depth; k++) tprice=iCustomMa(AverageType,tprice,AveragePeriod,i,k);
      MaBuffer[i]=tprice;
      if(i>0)
        {
...
string methodNames[]={"SMA","EMA","Double smoothed EMA","Double EMA","Tripple EMA","Smoothed MA","Linear weighted MA","Parabolic weighted MA","Alexander MA","Hull MA","Triangular MA","Sine weighted MA","Linear regression","IE/2","NonLag MA","Zero lag EMA","Leader EMA"};
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
